...
1.jpg        → ไอคอน/โลโก้เว็บ (อ้างอิงไว้ใน index.html แล้ว แต่ยังไม่เคยอัปโหลดไฟล์จริง — ไม่กระทบการทำงาน)
...
- **`1.jpg`** ยังไม่เคยอัปโหลดไฟล์จริงเข้า repo (แค่จุดเล็กๆ ไม่กระทบการทำงาน ใช้ favicon default แทน)
...
4. **ระวังเรื่อง Google Sheets auto-format ตัวเลข** — ค่าที่ดูเหมือนตัวเลขแต่ต้องการเก็บเป็น string เป๊ะๆ (เช่น PIN, เบอร์โทรที่ขึ้นต้นด้วย 0) ต้องบังคับ `setNumberFormat('@')` ก่อน `setValue()` เสมอ ไม่งั้น Sheets จะตัดเลข 0 นำหน้าทิ้งเอง (เจอบั๊กนี้มาแล้วจริงกับ PIN `05032540`)
